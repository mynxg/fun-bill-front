
import { Line } from '@ant-design/plots';
import React from 'react';
import ReactDOM from 'react-dom';
import {
    listByQueryActiveCountUsingGET,
    queryGenderCountUsingGET,
    listByQueryRegisterCounttUsingGET,
    listByQueryBillCountAndAmountUsingGET,
} from '@/services/report/userRank/userRankController';

const userActivityResultStr = await listByQueryActiveCountUsingGET({
    pageNum: 1,
    pageSize: 10,
});

const userActivityData = userActivityResultStr.data?.map((item) => {
    return {
        时间: `${item.userName} - ${item.loginTime}`,
        距离上次登录天数: item.lastLoginDays,
    }
});


const UserActivity: React.FC = () => {
    //   const data = [
    //     { year: '1991', value: 3 },
    //     { year: '1992', value: 4 },
    //     { year: '1993', value: 3.5 },
    //     { year: '1994', value: 5 },
    //     { year: '1995', value: 4.9 },
    //     { year: '1996', value: 6 },
    //     { year: '1997', value: 7 },
    //     { year: '1998', value: 9 },
    //     { year: '1999', value: 13 },
    //   ];
    const data = userActivityData;
    const config = {
        data,
        xField: '时间',
        yField: '距离上次登录天数',
        point: {
            shapeField: 'square',
            sizeField: 4,
        },
        interaction: {
            tooltip: {
                marker: false,
            },
        },
        style: {
            lineWidth: 2,
        },
    };
    return <Line {...config} />;
};

export default UserActivity;

